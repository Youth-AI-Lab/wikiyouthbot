# Superbot, the chatbot of the school bullying group

WikiYouthBot intensive week, La Rochelle, July 2026.
Working document: a full transcription of what the group produced, taken from the handwritten cards, the persona cards, the reference question and answer sheet, the mascot drawings and the digital artefacts. This English version is a translation. The French version holds the participants' original wording.

Reading the handwriting: uncertain passages are marked [?].

---

## Identity of the chatbot

- Name: Superbot.
- Theme given to the chatbot: supporting pupils facing bullying.
- Geographical scope written by the group: mainland France and the overseas territories.
- Mascot: Bouclebot, a standing figure holding a large red, blue and yellow shield marked with a chevron, drawn and coloured with felt pens, captioned "our mascot". A first proposal named Jean-Pierre was crossed out by the group.

Behaviour instructions written by the group, in their own words:

- Stop inventing sources.
- Keep a list of reliable sources.
- Stay on mainland France and the overseas territories.

Those three instructions are short, and that matters: the first one alone was enough to pull the chatbot out of its fabricating behaviour. The group wrote it after seeing for themselves that the chatbot was producing sources that did not exist.

---

## The dataset

Handwritten cards gathered through documentary research and by interviewing the people present around the venue.

### What comes from official sources

| Question on the card | What the group wrote | Source stated |
| --- | --- | --- |
| Since when has the pHARe programme been generalised? | school year 2022: generalised to primary schools and lower secondary schools. School year 2023: extended to upper secondary schools, compulsory for all establishments | education.gouv.fr |
| Since when has the 3018 helpline existed? | the 3018 has existed since 27 September 2023 | source line left blank |
| Which arrangements allow pupils to speak up, and what share of establishments are equipped? | primary: pHARe and a questionnaire. Lower secondary: pHARe and pupil ambassadors. Upper secondary: pHARe and a questionnaire. The figure is 96 % of establishments equipped with these arrangements | info.gouv.fr and the ministry statistics department |
| Which arrangements allow pupils to speak up? | the 3018, a free helpline; the chat in the app; a digital safe to store evidence of bullying; a self-assessment questionnaire to encourage people to speak about it | info.gouv.fr |

### What comes from the field enquiry

| Question on the card | What the group wrote |
| --- | --- |
| Out of 5 people around you, how many know the 3018? | 3 yes, 1 no |
| Ask 10 people whether they know the 3018 | tally marks, count to be confirmed |
| Ask 5 adults for one simple gesture that helps | write down what is happening; talk about it (2 answers); talk about it and work out what to do to avoid it; reassure the person targeted, restore their self-confidence, give them ways to answer bullying and not to be afraid of others or of adults; train young people and adults |
| Out of 5 people, how many have already seen someone being excluded? | 5 out of 5 |
| Ask 5 adults where they would direct a young person who is being bullied | to a parent, or an adult in general; the police, an association; first to someone responsible in the school; the school psychologist, the school nurse |
| Out of 5 people, how many know who to talk to in case of bullying? | take out the phone and call 118 [?]; my parents (2 answers); no, I do not know; my therapist |
| Ask adults whether they have ever faced a bullying problem, and how they reacted | no, alert the police; yes, in the street, reaction: ignore it; yes, went with their child to see the person responsible in the school; yes, alert the adults |
| Out of 5 people, how many think a witness who reacts helps? | yes, because if the person does not care it can go further (2 people); yes it can help, but also no, because it can push the bully to bully more (2 people) |
| Out of all the young people around you, how many say they have been bullied at least once at school? | 2 |
| How many say they have faced mockery or racism [?] at primary, lower or upper secondary school? | 4 |

The nuanced answer about the witness, where four people split between "it helps" and "it can make things worse", is the most useful material of the set: it fed the chatbot's answer on the role of the witness directly.

---

## The reference question and answer sheet

One completed sheet, with a format variant invented by the group: the reference answer box was split into two columns, what we want and what we don't want.

Theme: school bullying.
Question: what sanctions exist at school against the different types of bullying?
What we want: the sanctions that apply in Charente-Maritime, with reliable sources.
What we don't want: that it invents; that it says it knows nothing; that it plays down the seriousness [?]; that it says it has nothing.
Prohibitions or constraints: not checking whether the content it retrieves is AI-generated; using Wikipedia.

The what we want and what we don't want split is the group's own find, and is worth passing on to the consortium: it makes the template easier to use when stating a quality requirement.

---

## The personas

Four cards, three of them complete. The group covered the four positions of a bullying situation: the target, the witness, the bully, and the adult in the school.

### Joseph [surname illegible], 45

- Occupation: school supervisor. He lives with his parents.
- Favourite apps: McDonald's, Facebook, WhatsApp. Likelihood of using the chatbot: 2 out of 5.
- Cursors: knowledge of the cause 5 out of 5, civic engagement 0 out of 5, patience 0 out of 5, perception of technology 0 out of 5.
- History: he lives in Margny [?], he does not earn enough, he is bullied.
- Expectations: he does not want to be a bully and he cannot understand bullying.
- Typical prompt and expected tone: boxes left blank.

### Monsieur Non-Frus [?], 17, the witness

- Occupation: works at the school, he has repeated a year. His mother has died.
- Favourite apps: YouTube, Snap, Spotify. Likelihood of using the chatbot: 2 out of 5.
- Cursors: knowledge of the cause 4 out of 5, civic engagement 5 out of 5, patience 5 out of 5, perception of technology 2 out of 5.
- History: he saw his friend being bullied.
- Expectations: he is looking for how to help the person being bullied.
- Typical prompt: "What can I do, as a witness, to help someone who is being bullied?"

### Valentin Amstar, 14, the bully

- Occupation: volunteer junior firefighter and pupil, he has not repeated a year. He has lost a grandparent, his parents are together.
- Favourite apps: Snap, TikTok, Spotify. Likelihood of using the chatbot: 2 out of 5.
- Cursors: knowledge of the cause 3 out of 5, civic engagement 5 out of 5, patience 2 out of 5, perception of technology 3 out of 5.
- History: he went through his whole schooling. He was bullied, he could not say so, and he became a bully. He is gay.
- Expectations: he is looking for more information on sanctions, and to understand how bullying works.
- Typical prompt: "What sanctions do I risk if I have bullied someone?"

This is the strongest card of the set: the group built a bully who had first been bullied, and gave him a sincere question rather than a provocation.

### Ariana Petite, 12, the target

- Favourite apps: Snap, YouTube, Spotify. Likelihood of using the chatbot: 2 out of 5.
- Cursors: knowledge of the cause 2 out of 5, civic engagement 4 out of 5, patience 1 out of 5, perception of technology 2 out of 5.
- The rest of the card, occupation, family situation, history, expectations, prompt and tone, was left blank.

---

## The digital artefacts

### Superbot, the public page

Welcome line written by the group: "Hi! I answer questions about school bullying in France: the arrangements, the 3018, the pHARe programme, who to talk to. Ask your question."

An embedded knowledge base of six reference answers written from the dataset, each with its source displayed:

1. What is the pHARe programme? Source: education.gouv.fr.
2. Since when has the 3018 existed? An answer that distinguishes the earlier existence of the number for cyberbullying, the interministerial plan of 27 September 2023 that made it the single number, and the replacement of the 3020 on 1 January 2024. Source: education.gouv.fr and e-Enfance.
3. Which arrangements allow pupils to speak up? With the figure of 96 % of schools engaged in pHARe certification as of 30 June 2025. Source: info.gouv.fr.
4. Who can a young victim turn to? Source displayed: the group's enquiry with 5 adults.
5. How can you help when you are a witness? Source displayed: the group's enquiry with 5 adults.
6. Why do people bully others? An answer on the rejection of a difference, the designation of a scapegoat, group pressure and the peak of risk at the end of primary and in lower secondary school. Source: education.gouv.fr, the brochure on bullying between pupils.

Two of the six answers cite the group's enquiry as their source. The dataset built by the participants therefore became a displayed source on the same footing as education.gouv.fr, which is exactly the purpose of the dataset step.

Response mechanisms built during the week:

- A DISTRESS list of expressions of personal difficulty, which takes priority over any other classification: as soon as one of them is recognised, the chatbot leaves every other behaviour and directs the person to a trusted adult.
- A HESITATION list for expressed blocks: I do not dare, I am ashamed, nobody believes me, it will make things worse.
- A CONTEMPT list, added after a test by the group: it catches expressions of superiority over other pupils, a case the participants found themselves while testing the chatbot.
- A GREETINGS list with its own welcome answer.
- Approximate matching by Levenshtein distance, with tolerances by word length, to catch typing mistakes and uncertain spelling.
- A learning flow when a question is not recognised, with classification and filing of the new entry.
- A "copy my base" button to export and enrich the base durably.
- Read-aloud with a voice selector.
- An API key field for free questions, the key staying on the device.
- A permanent footer: 3018, the free helpline against bullying and online violence; 119, children in danger; and an invitation to talk to a trusted adult, a parent, a teacher, the school nurse or the head of year.

### The robot of the future

A section of the public page, built from the schematic drawn by the group, with a front view, a side view and the choice between castors or a hoverboard wheel.

Superbot as a robot is 50 cm wide and includes:

- hug arms;
- a night light;
- a tissue box;
- a phone arm to call for help;
- a backpack dispenser and printer;
- a sound output.

---

## Points to watch for the reporting

- Source vigilance: an earlier session produced fabricated sources, which were spotted and corrected. Any figure reused in the reporting must be attached to a real source, namely education.gouv.fr, info.gouv.fr, service-public.fr, the 3018 and e-Enfance, and the law of 2 March 2022.
- The personas contain sensitive biographical elements written by the participants, in particular about the sexual orientation of one persona and the death of a parent. They are faithful to their production and revealing of their understanding of the subject, but they call for measured handling in a document circulated to the consortium.
- Giving personal distress priority over every other response logic is the strongest design point of the project. That is what should be highlighted in the achievements section.
